...
const getInviteRoomCode = (location) => {
...
function Lobby() {
  ...
  const copyText = async (text, successMessage) => {
    try {
      ...
      setTimeout(() => setStatusMessage('Esperando oponente...'), 1600)
    } catch {
  ...
  useEffect(() => {
    const roomFromUrl = getInviteRoomCode(location)
    ...
    setError('')
    setStatusMessage('Abriendo invitación...')
    setView('joining')
    handleJoinRoom(roomFromUrl)
  }, [location.search, location.hash, mode])

  const handleCopyRoomId = () => {
    if (!roomId) return
    copyText(roomId, '¡Código copiado!')
  }

  const handleCopyInviteLink = () => {
    if (!roomId) return
    copyText(buildInviteUrl(roomId), '¡Enlace copiado!')
  }

  const buildInviteUrl = (roomCode = roomId) => {
    const { path, params } = getRouteParams(location)
    const nextParams = new URLSearchParams(params)
    if (roomCode) nextParams.set('room', roomCode)
    const query = nextParams.toString()
    return `${window.location.origin}${window.location.pathname}#${path}${query ? `?${query}` : ''}`
  }
      ...
      <div className="lobby-card">
        {view === 'select' && (
          <div className="lobby-options">
            ...
            <p className="lobby-help">Si abres un enlace de invitación, se rellenará el código automáticamente.</p>
        ...
        {view === 'created' && (
          <div className="lobby-room">
            ...
            <div className="room-share-row">
              <button className="lobby-btn lobby-btn-copy" onClick={handleCopyInviteLink}>
                📋 Copiar enlace
              </button>
